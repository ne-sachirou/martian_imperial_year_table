...
def view(state, actions):
    ...
    return h("div", {}, [
        h("form", {}, [
            ...
            h("button", {"onclick": lambda event: actions["sol_number_to_imperial_date_time"](event)},
              "to ImperialDateTime")
        ]),
        h(
            "div", {},
            f"ImperialDateTime: {imperial_date_time.year}-{imperial_date_time.month}-{imperial_date_time.day}T{imperial_date_time.hour}:{imperial_date_time.minute}:{imperial_date_time.second}"
        )
    ])
...
def main():
    """Main."""
    state = {"sol_number": SolNumber(0), "sol_number_ref": "sol_number_ref" + Math.random()}
    actions = {
        "sol_number_to_imperial_date_time":
        lambda event: lambda state: action_sol_number_to_imperial_date_time(state, event)
    }
